...
app=wx.App()    #初始化
win=wx.Frame(None,title='PDF文件与学院图标合成软件V1.0',size=(420,350))
bkg = wx.Panel(win)
hbox = wx.BoxSizer()
filename = wx.TextCtrl(bkg,value='运输职院PDF文件与学院图标合成软件V1.0')
...
def load(event): 
    ...
    #打开水印pdf文件
    water = PyPDF2.PdfFileReader(open('jpg.pdf','rb'))
    #创建新的pdf，并写入 
    ...
    newfile = open('addwater2.pdf','wb')
    pdfwriter.write(newfile)
    cqgy_file.close()
    newfile.close()
    contents.Clear()
    contents.AppendText('\n\n\n\n\n          恭喜！PDF文件与学院图标合成成功')
loadButton = wx.Button(bkg, label='开始合成学院图标')
...
def save(event):
    for filenames in os.walk('.'):
        ...
        b = '已经合成学院图标' #+ str(filelist) + '\n'
    webbrowser.open('addwater2'+'.pdf')
    contents.Clear()
